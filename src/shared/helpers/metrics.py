...
def ssim(a: np.ndarray, b: np.ndarray) -> float:
    ...
    if len(b) == 0:
        raise EmptyVector(f"{b} is empty")
    data_range = 255 if a.dtype == np.uint8 else 65535
    return structural_similarity(a.flatten(), b.flatten(), data_range=data_range)
...
def entropy(img_hist: np.ndarray) -> float:
    ...
    """
    if len(img_hist) == 0:
        raise EmptyVector(f"{img_hist} is empty")

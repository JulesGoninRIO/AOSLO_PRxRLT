...
from src.PostProc_Pipe.CellDetection.ComputerVisionEngine.cv_functions._local_maxima import get_photoreceptor_centers
from src.PostProc_Pipe.CellDetection.ComputerVisionEngine.helpers import concatenate_arrays_with_unique_elements#, get_array_statistics
from .global_constants import NON_EXISTENT_VERTEX, GOOD_NUMBER_OF_NEIGHBORS, \
...
class VoronoiDiagram:
    ...
    region_colors = {GOOD_NUMBER_OF_NEIGHBORS - 2: 'blue',
    ...
    def __init__(self, points: np.ndarray, image: np.ndarray = None) -> None:
        ...
        except (scipy.spatial.qhull.QhullError, IndexError, ValueError):
        ...
        for i in range(self.number_of_regions):
            self.number_of_neighbors[i] = np.count_nonzero(self._diagram.ridge_points[:, 0] == i) \
                                          + np.count_nonzero(self._diagram.ridge_points[:, 1] == i)
        self.number_of_good_cells: int = np.count_nonzero(self.number_of_neighbors == GOOD_NUMBER_OF_NEIGHBORS)
    ...
    def get_neighbor_idx(self, point_ind: int) -> np.array:
        ...
        """
        return np.concatenate((self._diagram.ridge_points[self._diagram.ridge_points[:, 0] == point_ind, 1],
    ...
    def get_distances_to_neighbors(self, point_ind: int) -> np.ndarray:
        ...
        """
        return np.concatenate((self._distances_between_neighbors[self._diagram.ridge_points[:, 0] == point_ind],
    ...
    def predict_new_cells(self) -> np.ndarray:
        ...
        updated_cell_coordinates = concatenate_arrays_with_unique_elements(updated_cell_coordinates,
    ...
    def draw_voronoi(self,
        ...
        for i in range(len(correct_facets)) :
            ...
            facet = np.array(correct_facets[i], np.int)
            ifacet_arr = []
            for f in facet :
                ifacet_arr.append(f)

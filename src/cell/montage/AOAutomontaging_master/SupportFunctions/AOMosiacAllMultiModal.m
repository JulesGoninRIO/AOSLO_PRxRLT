...
if(~AppendToExisting)
    ...
    [f_all, d_all] = calculateFeatures(imageFilename, parallelFlag, pixelScale, featureType, MN, N);
else
...
% d_all = load('d_all.mat');
% d_all = d_all.d_all;
% f_all = load('f_all.mat');
% f_all = f_all.f_all;
...
save(fullfile(outputDir,'AOMontageSave'),'LocXY','inData','TransType',...
    ...
    'RelativeTransformToRef', 'minXAll', 'maxXAll', 'minYAll', 'maxYAll');
        ...
        if ~isempty(imageFilename{m,AllRefIndex(i)})
            ...
            for n = RefChains{i}
                if ~isempty(imageFilename{m,n})
                    ...
                    %save to file labelled with the reference of the first
                    %image of the chain
%                     saveFileName=[name,'_aligned_to_ref',num2str(i),'_m',num2str(m),'.tif'];
                    saveFileName=[name,'_aligned_to_ref',num2str(ref),'_m',num2str(m),'.tif'];
            ...
            if(NumOfRefs > 1)%only necessary if more than one piece
                saveFileName = strcat('ref_',num2str(ref),'_combined_m',num2str(m),'.tif');

...
if strcmp(device_mode, 'multi_modal')
    ...
    if strcmp(posFileLoc(end-3:end), '.csv')
        lut = readtable(posFileLoc, 'ReadVariableNames', false);
        ...
        for n = 1:N
            ...
            number = regexp(inData{1,n}, '_\d+_', 'match');
            number = strrep(number, '_', '');
            lutArray = table2array(lut(:,1));
            lutArray = cellfun(@(x) strsplit(x, ';'), lutArray, 'UniformOutput', false);
%             number_str = num2str(number{1,1});  % Add a semicolon to the end of the number
            indice = find(cellfun(@(x) strcmp(x{1}, number), lutArray));
%             indice = find(lutArray == str2double(number));
%             lutArray_split = cellfun(@(x) strsplit(x, ';'), lutArray, 'UniformOutput', false);
%             lutArray_numeric = cellfun(@(x) str2double(x), lutArray_split, 'UniformOutput', false);
%             indices = cellfun(@(x) find(x == str2double(number)), lutArray_numeric, 'UniformOutput', false);
%             indice = find(cellfun(@(x) x == 1, indices));
%             indice = find(strcmp(lutArray_split, str2double(number)));
            ID{n} = lutArray{indice}{1};
            LocXY(:,n) = [str2num(lutArray{indice}{2}); -str2num(lutArray{indice}{3})];
            pixelScale(n) = str2num(lutArray{indice}{4});
            parts = strsplit(imageFilename{1,n}, '\');
            filename = parts{end};
            if ~isempty(strfind(filename, '_OD_'));
                eyeSide{n} = 'OD';
            elseif ~isempty(strfind(filename, '_OS_'));
                eyeSide{n} = 'OS';
            else
                eyeSide{n} = '';
            end
        ...
        C = cell2mat(readtable(posFileLoc));

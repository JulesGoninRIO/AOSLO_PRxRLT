...
tob.setTag('BitsPerSample',8); %maybe change to 16
...
if(isa(imageToSave,'double') || isa(imageToSave,'single'))
    imageToSave = uint8(round(imageToSave*255)); % if change to 16, here put uint16
end
...
try
    ...
    tob.write(typecast(imageToSave,'uint8'));
end
tob.close
end
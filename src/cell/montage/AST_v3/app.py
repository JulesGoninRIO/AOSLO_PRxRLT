...
class App(tk.Tk):
    ...
    def __init__(self, directory: str = None):
        """
        Initialize the GUI Window with needed parameters
        ...
        super().__init__()
        self.title('Acquisition Support Tool V2: unconnected component correction')
        ...
        # Set up base parameters for compoenent transparancy, the transparancy
    ...
    def __monitor(self) -> Tuple[int, int]:
        ...
        if len(monitors)>1:
            print("You have mutliple screens: will not use the principal monitor "\
                  "as for image size but the additional screen.")
        print("Please open the window on the additional screen in full size")
        monitor = monitors[-1]
    ...
    def __load_montage_images(self) -> None:
        ...
        if filenames_left_side and filenames_right_side:
            print("The images are from left and right eye, cannot process them")
            self.__shutdown()
            ...
            print("No image in the directory to find whether it is a left or "\
                  "right eye.")
    ...
    def __show_connected_components(self):
        ...
        if selected_components:
            selected_components = [os.path.basename(comp) for comp in selected_components]
            self.connected_components_sorted = selected_components + \
                [comp for comp in self.connected_components_sorted if comp not in selected_components]

...
class DragManager():
    ...
    def __init__(self, montage_dir: Path, canvas: tk.Canvas, image_name: str, \
                 ...
                 dragged_transparancy: float = 0.75, angle: int = 0, scale: int = 1):
        """
        ...
        image_width = im.shape[1]*IMAGE_SIZE_ON_FUNDUS*image_width # ADDED
        ...
        self.size = round(image_width * self.scale), round(ih_adapted * self.scale)
    ...
    def redraw(self, scale: float):
        ...
        new_size = round(iw * self.scale), round(ih * self.scale)
    ...
    def get_coordinates(self) -> Tuple[float, float, Tuple[int, int], float, int]:
        ...
        return self.xpos, self.ypos, self.image.size, self.scale, self.angle
    ...
    def move_canvas(self, dx: int = 0, dy: int = 0):
        """
    ...
    # def unbind_drag_drop_tags(self):
    #     """
    #     Unbind tags of drag and drop to the Canvas object
    #     """
    #     self.canvas.tag_unbind(self.image_obj, '<Button1-Motion>')
    #     self.canvas.tag_unbind(self.image_obj, '<ButtonRelease-1>')
